...
# Config (defaults)
# ---------------------
# Name of the gambling channel to lock/unlock (case-insensitive match)
GAMBLING_CHANNEL_NAME = "gambling"
# Hours to lock the channel after 3 warns
LOCKOUT_HOURS = 24
...
class Gambling(commands.Cog, name="Gambling"):
    ...
    def _find_gambling_channel(self, guild: discord.Guild) -> discord.TextChannel | None:
        for ch in guild.text_channels:
            if GAMBLING_CHANNEL_NAME in ch.name.lower():
                return ch
        return None
    ...
    async def _lock_channel(self, guild: discord.Guild, user: discord.Member):
        ...
        ch = self._find_gambling_channel(guild)
        if ch is None:
            return
    ...
    async def _unlock_channel(self, guild: discord.Guild, user_id: int):
        ...
        data = load_data()
        user_data = get_user_data(data, guild.id, user_id)
        user_data["warns"] = 0
        ...
        if died:
            ...
            if warns >= MAX_WARNS:
                ...
                self.bot.loop.create_task(
                    self._unlock_channel(interaction.guild, interaction.user.id)
                )
    ...
    async def gambling_pardon(self, interaction: discord.Interaction, user: discord.Member):
        ...
        ch = self._find_gambling_channel(interaction.guild)
        if ch:
